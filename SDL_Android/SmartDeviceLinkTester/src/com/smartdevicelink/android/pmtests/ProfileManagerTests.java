package com.smartdevicelink.android.pmtests;
...
import com.smartdevicelink.proxy.rpc.enums.ProfileState;
import com.smartdevicelink.proxy.rpc.enums.Result;
import com.smartdevicelink.proxy.rpc.pm.AddProfile;
import com.smartdevicelink.proxy.rpc.pm.HashTableKeys;
import com.smartdevicelink.proxy.rpc.pm.OnProfileClosed;
import com.smartdevicelink.proxy.rpc.pm.OnProfileStateChanged;
...
public class ProfileManagerTests {
    ...
    private ProfileState mExpectedState = ProfileState.BACKGROUND;
    ...
    public void runProfileManagerTests() {
        mLogAdapter.logMessage("Starting Profile Manager Tests", true);
        ...
        new Thread() {
            public void run() {
                ...
                if (mFailedTests.size() > 0) {
                    ...
                    mLogAdapter.logMessage("Failed tests: " + failedTests.toString(), true);
                } else {
                    mLogAdapter.logMessage("All tests passed", true);
                }
        ...
        if (mTestsInProgress) {
            synchronized (mRequestsSuccessExpected) {
                if (message instanceof RPCResponse) {
                    ...
                    if (mRequestsSuccessExpected.containsKey(corrId)) {
                        if (mRequestsSuccessExpected.get(corrId)) {
                            if (response.getResultCode() != Result.SUCCESS) {
                                mLocalErrorCounter++;
                                mLogAdapter
                                        .logMessage("Error occured " + response.toString(), true);
                            }
                        } else {
                            if (response.getResultCode() == Result.SUCCESS) {
                                mLocalErrorCounter++;
                                mLogAdapter.logMessage("Error occured (unexpected success) "
                    ...
                    if (message.getParameters(HashTableKeys.MSG_PROFILE_TO_APP) != null) {
                        String msg = new String(
                                ((OnSendProfileToAppMessage) message).getMessage())
                                .replace("\0", "");
                        switch (mCurrentTest) {
                            case 19:                                
                                mLogAdapter.logMessage("received: " + msg, true);
                ...
                } else if ((message instanceof OnProfileStateChanged) && (mCurrentTest == 41)) {
                    if (((OnProfileStateChanged) message).getState() != mExpectedState) {
                        mLocalErrorCounter++;
                        mLogAdapter.logMessage(
                                "Error occured (received profile state != expected)",
                                true);
                    }
                    mStateReceived = true;
                }
    ...
    private void compareMessages(String receivedMsg, String expectedMsg) {
        if (!receivedMsg.equals(expectedMsg)) {
            mLocalErrorCounter++;
            mLogAdapter.logMessage(
                    "Error occured (received message != sent message)",
                    true);
        } 
    ...
    private void checkAndResetLocalErrors(int testNumber) {
        if (mLocalErrorCounter != 0) {
            mLogAdapter.logMessage("test " + testNumber + " failed", Log.ERROR, true);
            mFailedTests.add(testNumber);
        } else {
            mLogAdapter.logMessage("test " + testNumber + " succeeded", true);
        }
     ...
     */
    private void test02() {
        mLogAdapter.logMessage("test02 started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 2;
     ...
     */
    private void test03() {
        mLogAdapter.logMessage("test03 started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 3;
     ...
     */
    private void test04() {
        mLogAdapter.logMessage("test04 started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 4;
     ...
     */
    private void test05() {
        mLogAdapter.logMessage("test05 started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 5;
     ...
     */
    private void test09() {
        mLogAdapter.logMessage("test09 started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 9;
     ...
     */
    private void test10() {
        mLogAdapter.logMessage("test10 started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 10;
     ...
     */
    private void test11() {
        mLogAdapter.logMessage("test11 started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 11;
     ...
     */
    private void test14() {
        mLogAdapter.logMessage("test14 started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 14;
     ...
     */
    private void test17() {
        mLogAdapter.logMessage("test17 started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 17;
     ...
     */
    private void test19() {
        mLogAdapter.logMessage("test19 (+21) started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 19;
     ...
     */
    private void test22() {
        mLogAdapter.logMessage("test22 started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 22;
     ...
     */
    private void test24() {
        mLogAdapter.logMessage("test24 started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 24;
     ...
     */
    private void test25() {
        mLogAdapter.logMessage("test25 started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 25;
     ...
     */
    private void test26() {
        mLogAdapter.logMessage("test26 (+ 27) started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 26;
     ...
     */
    private void test30() {
        mLogAdapter.logMessage("test30 (+ 31) started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 30;
     ...
     */
    private void test32() {
        mLogAdapter.logMessage("test32 started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 32;
     ...
     */
    private void test33() {
        mLogAdapter.logMessage("test33 started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 33;
     ...
     */
    private void test34() {
        mLogAdapter.logMessage("test34 started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 34;
     ...
     */
    private void test36() {
        mLogAdapter.logMessage("test36 started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 36;
     ...
     */
    private void test38() {
        mLogAdapter.logMessage("test38 started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 38;
     ...
     */
    private void test39() {
        mLogAdapter.logMessage("test39 started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 39;
     ...
     */
    private void test41() {
        mLogAdapter.logMessage("test41 started >>>>>>>>>>>>>>>>>>>>>>>>>>> ", true);
        mCurrentTest = 41;

        loadProfile(TEST_ECHO_PROFILE_NAME, true);
        waitForResponses();
        mExpectedState = ProfileState.BACKGROUND;
