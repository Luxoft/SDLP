	...
	public static JSONObject serializeHashtable(Hashtable<String, Object> hash) throws JSONException{
		...
		while (hashKeyIterator.hasNext()){
			...
			if (value instanceof RPCStruct) {
				...
				putObjectToJson(obj, key, value);
			}
		}
		return obj;
	}

    private static void putObjectToJson(JSONObject json, String key, Object value) throws JSONException {
        if (value.getClass().isArray()) {
            // TODO do we need multidimensional arrays support?
            JSONArray serializedValue = new JSONArray();
            for (int i = 0; i < Array.getLength(value); i++) {
                serializedValue.put(Array.get(value, i));
            }
            json.put(key, serializedValue);
        } else {
            json.put(key, value);
        }
    }
