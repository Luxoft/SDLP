			...
			if (data.length > MAX_DATA_SIZE) {
				...
				for (int i = 0; i < frameCount; i++) {
					if (i < (frameCount - 1)) {
                        ...
                        if (frameSequenceNumber == ProtocolFrameHeader.FrameDataFinalConsecutiveFrame) {
                            /**
                             * Because of byte overflow this may happen; then
                             * headunit will treat this frame as a final one
                             * (when it really isn't), and will ignore the rest
                             * of the frames. 
                             * TODO some proper solution should be implemented
                             */
                            frameSequenceNumber = (byte) (i + 2);
                        }
					} else {
