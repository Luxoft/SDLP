...
public class TcpDiscoverer {
    ...
    private InetAddress getInetFromInteger(int address) throws IOException {
    ...
    private InetAddress getBroadcastAddress() throws IOException {
        if (mContext != null) {
            WifiManager wifi = (WifiManager) mContext.getSystemService(Context.WIFI_SERVICE);
            DhcpInfo networkstate = wifi.getDhcpInfo();
            InetAddress inetaddr = getInetFromInteger(networkstate.ipAddress | (~networkstate.netmask));
            Log.i(TAG, "Broadcast: " + inetaddr);
            return inetaddr;
        }
        return null;
    }
    ...
    private Runnable mSearchLoop = new Runnable() {
        ...
        public void run() {
            ...
            InetAddress broadcastAddress, localAddress;
            try {
            ...
            while (!isTimeout(beginTime)) {
                ...
                if (sender.equals(localAddress)) {
                    // We should ignore our own broadcasts
                    Log.v(TAG, "got echo");
                    continue;
                }
                ...
                if (packet.getLength() == (MIN_PACKET_SIZE + NAME_LENGTH + UUID_LENGTH)) {
                    ...
                    device = new DiscoveredDevice(sender.getHostAddress(), port, name, uuid);
                } else {
