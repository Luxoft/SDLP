...
void TestSendingMsgProfile::onMessageReceived(const char *data, const int dataSize, const int appId)
{
    LOG4CPLUS_TRACE_METHOD(mLogger, __PRETTY_FUNCTION__);
    ...
    {
        mCallbacks->profileClosed(mAppId);
    }
}

void TestSendingMsgProfile::onAppStateChanged(const MobileAppState state, const int appId)
{
    LOG4CPLUS_TRACE_METHOD(mLogger, __PRETTY_FUNCTION__);
}
...
void TestSendingMsgProfile::onHmiRpcResponse(const int appId, NsSmartDeviceLinkRPC::SDLRPCResponse * response)
{
    LOG4CPLUS_TRACE_METHOD(mLogger, __PRETTY_FUNCTION__);
    LOG4CPLUS_INFO(mLogger, "Got response version = " << response->getProtocolVersion());
}

void TestSendingMsgProfile::onHmiRpcNotification(const int appId, NsSmartDeviceLinkRPC::SDLRPCMessage * notification)
{
    LOG4CPLUS_TRACE_METHOD(mLogger, __PRETTY_FUNCTION__);
}
