        ...
        const int appId)
{
    ...
    if (iter == mIncompleteLibsInfo.end())
    {
        if (frameNumber != 0)
        {
            ...
            pthread_mutex_unlock(&mMutex);
            return ProfileManagerResult(Result::INVALID_DATA, "Wrong order of frames");
        }
...
{
    LOG4CPLUS_TRACE_METHOD(mLogger, __PRETTY_FUNCTION__);
    ProfileManagerResult result(Result::SUCCESS);
    pthread_mutex_lock(&mMutex);
    if (!eraseFile(genPathToProfile(filePath, profileName)))
    {
        result.retcode = Result::INVALID_ID;
        result.description = "No library to remove for profile " + profileName;
    }
    pthread_mutex_unlock(&mMutex);
    return result;
}
